...
module fp_inv_sqrt_fast_stage (
  ...
  fp_add_fast add_sub(.clk(clk), .rst(rst), .a(three), .b(y_sq_by_x), .is_sub(1'b1), .sum(sub));
...
module fp_inv_sqrt_fast (
  ...
  generate
    genvar i;
    for (i = 0; i < NR_STAGES; i = i + 1) begin : fast_inv_sqrt_stage_gen
      fp_inv_sqrt_fast_stage inv_sqrt_stage (
        .clk(clk),
        .rst(rst),
        .x((i == 0) ? x : x_buffer[i]),
