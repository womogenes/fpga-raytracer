...
MIRROR_SPEC = (0.5, 0.25, 0.75)
BACK_LIGHT_EMIT = (0.4, 0.2, 0.1)
MIRROR_EXPECTED = tuple(a * b for a, b in zip(MIRROR_SPEC, BACK_LIGHT_EMIT))
...
TEST_MATERIALS = [
    ...
    Material(color=(1.0, 1.0, 1.0), emit_color=(0.0, 0.0, 0.0), spec_color=MIRROR_SPEC, smoothness=1.0, specular_prob=1.0),
    Material(color=(1.0, 1.0, 1.0), emit_color=BACK_LIGHT_EMIT, spec_color=(1.0, 1.0, 1.0), smoothness=0.0, specular_prob=0.0),
]
...
def write_mem(path: Path, packed_entries):
    path.write_text("\n".join(packed_entries) + "\n")


def material_hex_entries():
    entries = []
...
def runner():
    ...
    build_dir = proj_path / "sim" / "sim_build" / "ray_tracer_scene"
    build_data_dir = build_dir / "data"
    build_data_dir.mkdir(parents=True, exist_ok=True)
    write_mem(build_data_dir / "test_ray_tracer_mat_dict.mem", material_hex_entries())
    write_mem(build_data_dir / "test_ray_tracer_scene_buffer.mem", object_hex_entries())
    ...
    sources = [
        ...
        *glob.glob(f"{proj_path}/hdl/math/*.sv", recursive=True),
        ...
        proj_path / "sim" / "rtx" / "ray_tracer_scene_tb.sv",
    ]
    build_test_args = [
        "-Wno-WIDTHEXPAND",
        "-Wno-MULTIDRIVEN",
    ...
    runner.build(
        sources=sources,
        hdl_toplevel="ray_tracer_scene_tb",
        always=True,
